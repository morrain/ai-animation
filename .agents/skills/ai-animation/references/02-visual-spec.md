...
1. **生成 Shot #1 首尾双帧样板关键帧 (Pilot Keyframe Generation)**：
   - 严禁一次性全量调用生图工具生成全部分镜图片！在双帧控制模式（如 `vox` 风格）下，**必须且仅能通过 `run_command` 执行脚本 `python3 .agents/skills/ai-animation/scripts/generate_image.py <project_dir> --shot_id 1` 生成 Shot #1 的首帧 (`shot_01_first.png`) 和尾帧 (`shot_01_last.png`)** 作为完整样板组。
